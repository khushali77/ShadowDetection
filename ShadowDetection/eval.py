...
if __name__ == "__main__":
    pre_list = glob.glob('/content/drive/MyDrive/ECA-ShadowDetection/Testing/SBU/Tested_SBU/*.*')
    print(pre_list)
    X=0
    Y=0
    Z=0
    ...
    for i, path in tqdm.tqdm(enumerate(pre_list)):
        ...
        pre = cv2.imread(path)
        label = cv2.imread(path.replace('Tested_SBU', 'labels'))
        #label = cv2.imread('/content/drive/MyDrive/SBU_data/SBU-shadow/SBU-Test/just_labels')


        ber,sdr = BER(torch.from_numpy(label).float(), torch.from_numpy(pre).float())
        #mse=RMSE(label, pre)
        #Y=Y+mse
        X=X+ber
        Y=Y+sdr
