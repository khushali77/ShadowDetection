...
class ImageSelector(object):
    ...
    def __prepare_cluster_sets__hdbscan(self, files):
        ...
        Hdbascan = hdbscan.HDBSCAN(min_cluster_size=5,metric='manhattan').fit(all_dst)
    ...
    def __get_laplacian_scores(self, files, n_images):
        ...
        for image_i in n_images:
            img_file = files[n_images[image_i]][0]
            img = cv2.cvtColor(img_file, cv2.COLOR_BGR2GRAY)
    ...
    # def select_best_frames(self, input_key_frames, number_of_frames):
        ...
        if len(input_key_frames) >= 1:
            ...
            for index in selected_images_index:
                img = input_key_frames[index][0]
                filtered_images_list.append((img, index, input_key_frames[index][1]))
            ...
            for images in files_clusters_index_array:
                ...
                for image in images[0]:
                    #cv2.imwrite(os.path.join(path, str(image)+'.jpeg'),input_key_frames[image][0])
                    cv2.imwrite(os.path.join(path, str(image)+"_"+str(input_key_frames[image][1]) +'.jpeg'),input_key_frames[image][0])
                i=i+1
        else:
            # if the imput candidate frames are less than a single cluster.
            for idx, img in enumerate(input_key_frames):
                #filtered_images_list.append((img,idx))
                filtered_images_list.append((img,index, input_key_frames[index][1]))
