...
class FrameExtractor(object):
    ...
    def __process_frame(self, frame, prev_frame, frame_diffs, frames,i):
        """Function to calculate the difference between current frame and previous frame
        :param frame: frame from the video
        :type frame: numpy array
        :param prev_frame: previous frame from the video in LUV format
    ...
    def __extract_all_frames_from_video__(self, videopath):
        ...
        ret, frame = cap.read()
        # print("ret:",ret)
        # print("frame:",len(frame), frame.shape)
        # print(frame)
        i = 1
        chunk_no = 0
        while ret:
            ...
            for _ in range(0, self.max_frames_in_chunk):
                if ret:
                    ...
                    prev_frame, curr_frame = self.__process_frame(frame, prev_frame, frame_diffs, frames, i-1)
                    i = i + 1
                    ret, frame = cap.read()
            ...
            chunk_no = chunk_no + 1
            # print(chunk_no)
            # print(len(frames), len(frame_diffs))
    ...
    def __get_frames_in_local_maxima__(self, frames, frame_diffs):
        ...
        for frame_index in frame_indexes:
            extracted_key_frames.append((frames[frame_index - 1][0].frame, frames[frame_index - 1][1]))
    ...
    def extract_candidate_frames(self, videopath):
        ...
        # print("frames:",frame_extractor_from_video_generator)
        ...
        return self.fps, extracted_candidate_key_frames
